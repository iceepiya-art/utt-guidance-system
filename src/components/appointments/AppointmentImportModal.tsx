...
import { Appointment, School, TeamId, PhotoItem, AppointmentStatus } from '../../types';
import { useAuth } from '../../context/AuthContext';
import { createAppointment } from '../../firebase/dbService';
...
export const AppointmentImportModal: React.FC<AppointmentImportModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  ...
  const [items, setItems] = useState<RawAppointmentItem[]>([]);
  ...
  const itemsWithStatus = useMemo(() => {
    return items.map((item) => {
      ...
      const isDuplicate = existingAppointments.some((existing) => {
  ...
  const handleStartImport = async () => {
    ...
    for (let idx = 0; idx < candidates.length; idx++) {
      ...
      try {
        ...
        console.error('Failed to import appointment:', item.schoolName, err);
        failed++;
      }
      setProgress(idx + 1);
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                <span>นำเข้าตารางนัดหมายการแนะแนวจากระบบ UTT</span>
                <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-800">
                  {items.length} รายการ
                </span>
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                ดึงจาก https://www.utt.ac.th/guide/guidance_table.php พร้อมรูปถ่ายกิจกรรมครบถ้วน
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4">
          ...
          {loading ? (
            <div className="py-20 text-center space-y-3">
              ...
              <p className="text-sm font-semibold text-slate-700">กำลังโหลดรายการนัดหมายและรูปถ่าย...</p>
            </div>
          ) : result ? (
            <div className="py-10 text-center space-y-6 max-w-md mx-auto">
              <CheckCircle2 className="w-16 h-16 text-emerald-600 mx-auto animate-bounce" />
              <div>
                <h3 className="text-xl font-bold text-slate-800">นำเข้านัดหมายเสร็จสิ้นแล้ว!</h3>
                <p className="text-xs text-slate-500 mt-1">
                  รายการนัดหมายและรูปภาพได้ถูกบันทึกลงสู่ระบบเรียบร้อย
                </p>
              </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80">
                  <div className="text-xs text-slate-500">นัดหมายจาก UTT ทั้งหมด</div>
                ...
                <div className="p-3.5 rounded-2xl bg-purple-50 border border-purple-100">
                  <div className="text-xs text-purple-700 font-semibold">รูปถ่ายกิจกรรม</div>
                  <div className="text-xl font-bold text-purple-700 mt-0.5">{totalPhotosCount} รูป</div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2.5 items-center justify-between pt-1">
                <div className="relative flex-1 w-full">
                  ...
                  <input
                    ...
                    placeholder="ค้นหาชื่อโรงเรียน, อาจารย์ผู้บันทึก..."
                  ...
                  <table className="w-full text-left text-sm min-w-[780px]">
                    <thead className="bg-slate-50 text-slate-600 border-b border-slate-200 font-semibold text-xs sticky top-0 z-10">
                      <tr>
                        ...
                        <th className="py-2.5 px-3 min-w-[120px]">วันที่นัดหมาย</th>
                        <th className="py-2.5 px-3 min-w-[80px] text-center">สาย</th>
                        <th className="py-2.5 px-3 min-w-[120px]">ผู้บันทึก</th>
                        <th className="py-2.5 px-3 min-w-[110px]">สถานะเดิม</th>
                      ...
                      {filteredItems.map((item, idx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-2.5 px-3 font-semibold text-slate-800 text-sm">
                              {item.schoolName}
                            </td>
        ...
        {!result && (
          <div className="bg-white border-t border-slate-200 px-6 py-4 flex items-center justify-between">
            <div className="text-xs text-slate-500">
              {newItemsCount > 0 ? (
                <span>
                  จะนำเข้านัดหมายใหม่ <b className="text-emerald-700">{newItemsCount}</b> รายการ (ข้ามรายการเดิมอัตโนมัติ)
                </span>
              ) : (
                <span className="text-emerald-700 font-semibold">
                  ข้อมูลทั้ง 49 รายการมีครบในระบบแล้ว
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold rounded-xl shadow-xs disabled:opacity-40 transition-colors"
              >
                {importing ? (
                  <>
