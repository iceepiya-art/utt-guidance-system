...
export async function saveSubmissionAppointment(submission: Omit<DocumentSubmission, 'id'>, schedule: { date: string; startTime: string; endTime: string; note: string }, user: UserProfile, existingSubmission?: DocumentSubmission) {
  ...
  const conflict = await checkAppointmentConflict(schedule.date, schedule.startTime, schedule.endTime, submission.teamId, previous?.counselorId || user.id, existingSubmission?.appointmentId);
  ...
  batch.set(appointmentRef, { ...previous, ...schedule, note: schedule.note || previous?.note || '', schoolId: submission.schoolId, schoolName: submission.schoolName, teacherName: submission.teacherName, teacherPhone: submission.teacherPhone, teamId: submission.teamId, counselorId: previous?.counselorId || user.id, counselorName: previous?.counselorName || user.displayName, teamMemberNames: submission.submittedByNames?.join(', ') || submission.submittedByName, workType: 'แนะแนวการศึกษา', status: previous?.status === 'COMPLETED' ? 'COMPLETED' : 'CONFIRMED', source: 'DOCUMENT_SUBMISSION', submissionId: submissionRef.id, reminders: previous?.reminders || [], createdAt: previous?.createdAt || now, updatedAt: now, updatedBy: user.displayName, createdBy: previous?.createdBy || user.displayName }, { merge: true });
